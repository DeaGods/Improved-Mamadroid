...
def matrixcreation(DCVector, allnodes):  # 对于每个作为caller的api 计算它对其他api调用的概率 （每个caller下的所有状态传递总和为1）
    ...
    MarkovTransition = numpy.zeros(s, dtype=numpy.float32)
    MarkovFeats = numpy.zeros(s, dtype=numpy.float32)

    for s_idx in range(0, len(DCVector)):  # 修改变量名避免与shape变量s冲突
        for i in range(1, len(DCVector[s_idx])):
            MarkovTransition[DCVector[s_idx][0], DCVector[s_idx][i]] += 1  # 有相应的调用 对应的地方加1
